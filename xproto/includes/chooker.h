...
class CMemory
{
	public:
		...
		void* GetLibraryFromName( char *libname )
		{
				...
				DEBUG_PRINT("CHOOKER [%s] %s baseaddress=0x%x\n", __FUNCTION__, libname, baseaddress);
				if( ( baseaddress = dl_iterate_phdr( dl_callback, this ) ) )
				{
					DEBUG_PRINT("CHOOKER [%s] pass %s baseaddress=0x%x\n", __FUNCTION__, libname, baseaddress);
			...
			DEBUG_PRINT("CHOOKER [%s] error %s baseaddress=0x%x\n", __FUNCTION__, libname, baseaddress);
			return NULL;
		}
...
class CHooker
{
	...
	public:
		...
		{
			DEBUG_PRINT("CHOOKER [%s] 0x%x %i\n", __FUNCTION__, libaddr, issym);
			if(issym)
		...
		{
			DEBUG_PRINT("CHOOKER [%s] %s %i\n", __FUNCTION__, libname, issym);
			if(issym)
	...
	static int dl_callback( struct dl_phdr_info *info, size_t size, void *data )
	{
		...
		DEBUG_PRINT("CHOOKER [%s] %s 0x%x\n", __FUNCTION__, info->dlpi_name, data);
